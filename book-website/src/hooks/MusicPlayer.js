...
const MusicPlayer = () => {
    const tracks = [track2, track1, track3, track4, track5, track6, track7, track8, track9, track10, track11, track12];
    ...
    return (
      <div className="music-player">
        ...
        <div className="player-controls">
          ...
          <button onClick={handlePlayPause} aria-label={isPlaying ? "Пауза" : "Відтворити"}>
            <img src={isPlaying ? pauseButton : playButton} alt={isPlaying ? "Пауза" : "Відтворити"} />
          </button>
          <button onClick={handleNext} aria-label="Наступний трек">
            <img src={forwardButton} alt="Наступний трек" />
          </button>
        </div>
        <p>Трек {currentTrackIndex + 1} з {tracks.length}</p>
      </div>
    );
  };
